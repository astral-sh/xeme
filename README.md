...
Seven [current-source ASan campaigns](docs/validation/2026-09-11/streaming-input-bounds/ASAN.md) completed 2,170,731 executions and 68,897 retained-corpus replays without findings, alongside 13 focused policy regressions. Each campaign ran for 120 seconds; the earlier [six 600-second campaigns](docs/validation/2026-09-11/current-asan/) remain scoped to `be22a27`. The [stable PGO PBS distribution](docs/validation/2026-09-11/pgo-trial-results/) uses the selected streaming runtime, runs on glibc 2.17 and passes 1,024 threaded parses; its strict XML gates retain the same two callback assertions.
