...
These [historical normal-build measurements](https://github.com/astral-sh/oriole/tree/fe31da9b4050dfc901aa2fbd1080cb558e1c9f3f/docs/validation/2026-09-12/native-start-end-namespace-declaration) use six pinned XML projects, 4 KiB chunks and namespaces disabled on a shared Linux host. Times are medians of seven process medians; ratios are medians of paired ratios. The later merged QName optimization takes **1.1739× Expat’s native time and 1.0504× its CPython time** across the tuning suite. Individual outliers remain. The [benchmark guide](benchmarks/HILLCLIMB.md) separates those tuning inputs from a new five-project holdout.

The [compatibility guide](docs/compatibility.md) defines the current contract and continuous regression gates. Known strict failures remain: 391 Expat API configurations, two CPython text-grouping assertions and 960 shared W3C catalog rows. These are documented differences, not passing upstream tests. [Qualification evidence](docs/evidence/README.md) identifies tested source revisions and the controlled, opt-in Linux CPython 3.12.13 trial; it does not establish general production interchangeability.
