...
def library_artifact(messages: str, checkout: Path, target_dir: Path) -> Path:
    ...
    for line in messages.splitlines():
        ...
        if (
            ...
            or message["target"]["name"] != "oriole_expat"
        ):
            continue
        if (
            message["fresh"]
            or Path(message["manifest_path"]).resolve()
            != checkout / "crates/oriole_expat/Cargo.toml"
            or Path(message["target"]["src_path"]).resolve()
            != checkout / "crates/oriole_expat/src/lib.rs"
        ):
            raise ValueError("unexpected or cached C library artifact")
        artifacts.extend(
            Path(filename).resolve(strict=True)
            for filename in message["filenames"]
            if Path(filename).name == "liboriole_expat.so"
        )
    expected = target_dir / TARGET / "release/liboriole_expat.so"
