...
The corpus covers six XML uses selected before timing: code-generation registries, protocol descriptions, build metadata, SVG artwork, UI trees and XSLT source. Each project contributes one unmodified upstream file with its original license/notice, exact commit and SHA256 in `corpus-manifest.json`.
