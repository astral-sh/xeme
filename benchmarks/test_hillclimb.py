...
class FreshBuildTests(unittest.TestCase):
            ...
            json.dumps(
                {
                    ...
                    "manifest_path": str(checkout / "crates/xeme_expat/Cargo.toml"),
                    "target": {
                        "name": "xeme_expat",
                        "src_path": str(checkout / "crates/xeme_expat/src/lib.rs"),
                    },
                    "fresh": fresh,
    ...
    def test_configured_target_cannot_publish_old_host_artifact(self):
        for toolchain in [None, "ohm"]:
            with (
                ...
                stale = args.target_dir / "release/libxeme_expat.so"
                stale.parent.mkdir(parents=True)
                stale.write_bytes(b"old baseline")
                fresh = (
                    args.target_dir / hillclimb.TARGET / "release/libxeme_expat.so"
                )
                ...
                self.assertEqual(
                    (args.output / "libxeme_expat.so").read_bytes(),
                    b"fresh candidate",
                )
    ...
    def test_only_current_uncached_emitted_library_is_accepted(self):
        with tempfile.TemporaryDirectory() as temporary:
            args = self.checkout(Path(temporary), "candidate")
            library = args.target_dir / hillclimb.TARGET / "release/libxeme_expat.so"
