...
/// Keep a native publication's identity after its retained bytes are discarded.
#[derive(Clone, Copy, Debug)]
enum PublishedLocation {
    Position(Position),
    Native(NativeLocation),
    ResolvedNative {
        native: NativeLocation,
...
impl From<AdapterLocation> for PublishedLocation {
    fn from(location: AdapterLocation) -> Self {
        match location {
            AdapterLocation::Position(position) => Self::Position(position),
            AdapterLocation::Native(native) => Self::Native(native),
        }
    }
}
...
pub struct Parser {
    ...
    last_position: PublishedLocation,
    current_raw: String,
...
impl Parser {
    ...
    pub fn try_new_with_encoding_in(
        ...
        Ok(Self {
            ...
            last_position: PublishedLocation::Position(Position {
                line: 1,
                ..Position::default()
            }),
    ...
    fn next_delivery_into(&mut self, output: &mut EventOutput<'_>) -> Result<(), Error> {
        // No new prospective frame exists yet. Retained bytes still describe
        // the committed event, including after a caught panic before delivery.
        output.lazy_coordinates = output.lazy_coordinates && self.can_defer_coordinates();
        if !output.lazy_coordinates || self.sources[0].compaction_due() {
            self.materialize_coordinates();
        }
        self.next_delivery_scoped(output)
    }
    ...
    fn next_event_scoped(&mut self, output: &mut EventOutput<'_>) -> Result<(), Error> {
        if let Some(event) = self.finish_foreign_dtd() {
            self.last_position = PublishedLocation::Position(event.position);
        ...
        if result.is_ok() && output.has_frame() {
            self.last_position = output.frame.as_ref().unwrap().location_for_c().into();
        }
        result
    }
    ...
    pub fn position(&self) -> Position {
        match self.last_position {
            PublishedLocation::Position(position) => position,
            PublishedLocation::Native(native) => self.sources[0].native_position(native),
            PublishedLocation::ResolvedNative {
                native,
                line,
                column,
            } => Position {
    ...
    pub fn resolve_native_location_for_c(&mut self, native: NativeLocation) -> Option<Position> {
        if native.generation != self.event_recycling.generation() {
            return None;
        }
        match self.last_position {
            PublishedLocation::Native(current) if current == native => {
                let position = self.sources[0].checkpoint_native(native);
                self.last_position = PublishedLocation::ResolvedNative {
                    native,
                    line: position.line,
            ...
            PublishedLocation::ResolvedNative {
                native: current, ..
            } if current == native => Some(self.position()),
            _ => None,
        }
    }

    /// Resolve the one committed start before source mutation or deferred discard.
    fn materialize_coordinates(&mut self) {
        if let PublishedLocation::Native(native) = self.last_position {
            self.resolve_native_location_for_c(native)
                .expect("current native publication");
        }
        self.sources[0].materialize_coordinates();
        self.sources[0].compact();
    }
    ...
    /// Create a prospective location only for a selected bounded native frame.
    fn frame_location(&mut self, count: usize, lazy: bool) -> AdapterLocation {
        if lazy && self.can_defer_coordinates() && count > 0 && count <= arena::MAX_ARENA_BYTES {
    ...
    /// Leave a native frame's bytes intact until the wrapper commits its delivery.
    fn consume_frame(&mut self, count: usize, location: AdapterLocation) -> Result<(), Error> {
        if let AdapterLocation::Native(native) = location {
            assert_eq!(native.byte_index, self.source().byte_index());
            assert_eq!(native.byte_count, count);
            self.account_source(count)?;
            self.sources[0].consume_deferred(count);
            Ok(())
        } else {
            self.consume(count)
        }
    ...
    fn next_event_inner(&mut self, output: &mut EventOutput<'_>) -> Result<(), Error> {
        ...
        loop {
            ...
            if self.source().remaining().is_empty() {
                ...
                self.finish_conditional_source()?;
                self.materialize_coordinates();
                self.last_position = PublishedLocation::Position(self.here());
                if self.in_doctype {
                    return Err(
            ...
            let location = framed_end.or_else(|| {
                output
                    .frame
                    .as_ref()
                    .filter(|frame| frame.is_active())
                    .map(|frame| frame.location_for_c())
            });
            if let Some(location) = location {
                self.consume_frame(end, location)?;
            } else {
                self.consume(end)?;
            }
