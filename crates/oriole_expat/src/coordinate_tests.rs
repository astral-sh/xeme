...
#[derive(Default)]
struct DiscardBoundary {
    parser: XML_Parser,
    text_calls: usize,
    end_calls: usize,
}

unsafe extern "C" fn boundary_text(data: *mut c_void, bytes: *const c_char, len: c_int) {
    // SAFETY: Callback data is live; only raw state projections cross getter calls.
    unsafe {
        if len != 4 || std::slice::from_raw_parts(bytes.cast::<u8>(), 4) != "éé".as_bytes() {
            return;
        }
        let state = data.cast::<DiscardBoundary>();
        let parser = (*state).parser;
        (*state).text_calls += 1;
        let location = (*parser).position;
        assert!(matches!(location, AdapterLocation::Native(_)));
        assert_eq!(XML_GetCurrentByteIndex(parser), 65_533);
        assert_eq!(XML_GetCurrentByteCount(parser), 4);
        assert_eq!((*parser).position, location);
        let mut offset = -1;
        let mut size = -1;
        let context = XML_GetInputContext(parser, &mut offset, &mut size);
        assert!(!context.is_null() && offset >= 0 && size >= offset + 4);
        assert_eq!(bytes, context.cast::<c_char>().add(offset as usize));
        for _ in 0..2 {
            assert_eq!(XML_GetCurrentLineNumber(parser), 2);
            assert_eq!(XML_GetCurrentColumnNumber(parser), 3);
        }
        assert_eq!(XML_StopParser(parser, 1), OK);
        // This native Text crosses Source's 64 KiB discard boundary. Its C owner
        // stays live through coordinate resolution and suspension; resume then
        // materializes the checkpoint before delivering the following end tag.
        assert_eq!(
            std::slice::from_raw_parts(bytes.cast::<u8>(), 4),
            "éé".as_bytes()
        );
    }
}

unsafe extern "C" fn boundary_end(data: *mut c_void, name: *const c_char) {
    // SAFETY: The detached end-name owner survives all getter reentry below.
    unsafe {
        ...
        assert_eq!(XML_GetCurrentByteIndex(parser), 65_537);
        assert_eq!(XML_GetCurrentByteCount(parser), 4);
        assert_eq!(XML_GetCurrentLineNumber(parser), 2);
        assert_eq!(XML_GetCurrentColumnNumber(parser), 5);
        assert_eq!(CStr::from_ptr(name).to_bytes(), b"n");
    }
}

#[test]
fn native_text_getters_survive_source_discard_after_suspension() {
    let input = format!("<r>{}\r\n<n>éé</n></r>", "x".repeat(65_525));
    for buffered in [false, true] {
        // SAFETY: Test-owned input/state live through parsing, callbacks and resume.
        unsafe {
            let parser = XML_ParserCreate(ptr::null());
            assert!(!parser.is_null());
            let mut state = DiscardBoundary {
                parser,
                ..DiscardBoundary::default()
            };
            XML_SetUserData(parser, ptr::from_mut(&mut state).cast());
            XML_SetCharacterDataHandler(parser, Some(boundary_text));
            XML_SetEndElementHandler(parser, Some(boundary_end));
            let status = if buffered {
                let buffer = XML_GetBuffer(parser, input.len() as c_int);
                assert!(!buffer.is_null());
                ptr::copy_nonoverlapping(input.as_ptr(), buffer.cast(), input.len());
            ...
            assert_eq!(state.text_calls, 1);
            assert_eq!(state.end_calls, 0);
            assert_eq!(XML_GetCurrentLineNumber(parser), 2);
            assert_eq!(XML_GetCurrentColumnNumber(parser), 3);
            // Refresh the user-data pointer after observing the callback state.
            XML_SetUserData(parser, ptr::from_mut(&mut state).cast());
            assert_eq!(XML_ResumeParser(parser), OK);
            assert_eq!(state.end_calls, 1);
            assert_eq!(XML_GetCurrentByteIndex(parser), input.len() as c_long);
            assert_eq!(XML_GetCurrentLineNumber(parser), 2);
            assert_eq!(XML_GetCurrentColumnNumber(parser), 13);
            XML_ParserFree(parser);
        }
    }
}
