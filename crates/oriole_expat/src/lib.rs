...
pub struct XML_ParserStruct {
    ...
    default_pending: VecDeque<String>,
    family: Arc<FamilyBudget>,
    child_depth: usize,
...
/// Copy a nullable caller-owned C string before mutating parser storage.
unsafe fn copy_string(value: *const c_char) -> Result<Option<String>, ()> {
    if value.is_null() {
        return Ok(None);
    }
    // SAFETY: The C caller supplies a readable NUL-terminated string.
...
fn cstring(value: String) -> CString {
    // XML forbids NUL, and names/values originate in the validated core parser.
    CString::new(value).expect("XML parser produced a NUL character")
}

fn optional_cstring(value: Option<String>) -> Option<CString> {
    value.map(cstring)
}
...
unsafe fn create(encoding: *const c_char, separator: Option<char>) -> XML_Parser {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Caller follows the nullable C string contract.
        let Ok(encoding) = (unsafe { copy_string(encoding) }) else {
            return ptr::null_mut();
        };
        let config = Config {
            encoding,
        ...
        let core = Parser::new(config.clone());
        let position = core.position();
        Box::into_raw(Box::new(XML_ParserStruct {
            ...
            default_pending: VecDeque::new(),
            family: Arc::new(FamilyBudget::default()),
            child_depth: 0,
...
pub unsafe extern "C" fn XML_ParserCreateNS(
    ...
    // SAFETY: Forward the C string contract to create.
    unsafe { create(encoding, Some(char::from(separator as u8))) }
}
...
pub unsafe extern "C" fn XML_ParserCreate_MM(
    ...
    // A custom suite must own *all* allocations. The Rust core does not yet support
    // per-parser allocators; reject this mode instead of violating that contract.
    if !suite.is_null() {
        return ptr::null_mut();
    }
    // SAFETY: A non-null separator points to at least one readable character.
    let separator = if separator.is_null() {
        None
    } else {
        // SAFETY: Non-null separator points to one readable character.
        Some(char::from(unsafe { *separator } as u8))
    };
    // SAFETY: Forward the C string contract to create.
    unsafe { create(encoding, separator) }
}
...
unsafe fn release_encoding(parser: XML_Parser) {
    ...
    unsafe {
        ...
        if let Some(release) = release { release(data); }
    }
}
...
pub unsafe extern "C" fn XML_ParserReset(parser: XML_Parser, encoding: *const c_char) -> u8 {
    ...
    unsafe {
            ...
            if (*parser).child_depth != 0 { return 0; }
            (*parser).busy = true;
    }
    let result = catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Scalar/core updates occur outside the encoding release callback.
        unsafe {
            release_encoding(parser);
            if (*parser).pending_free { return ERROR; }
            let Ok(encoding) = copy_string(encoding) else {
                (*parser).error = INVALID_ARGUMENT;
                return 0;
            };
            (*parser).config.encoding = encoding;
            (*parser).core = Parser::new((*parser).config.clone());
            (*parser).position = (*parser).core.position();
            // Expat preserves namespace triplet and unknown-encoding settings.
            ...
            (*parser).default_pending.clear();
            (*parser).family = Arc::new(FamilyBudget::default());
        }
        1
    }));
    // SAFETY: This reset owns the busy guard until completing or honoring Free.
    unsafe { u8::from(finish_operation(parser, result) == OK) }
}

/// Run callbacks after releasing all references to the opaque parser.
unsafe fn dispatch(parser: XML_Parser, kind: EventKind) {
    ...
    let callback_bytes = match &kind {
        EventKind::StartElement { name, attributes } => name.len() + attributes.iter().map(|a| a.name.len() + a.value.len()).sum::<usize>(),
        ...
        EventKind::EntityDeclaration { name, value, .. } => name.len() + value.as_ref().map_or(0, String::len),
        _ => 0,
    };
    // SAFETY: The shared budget contains atomic counters and never invokes user code.
    unsafe {
        if !charge(&Arc::as_ref(&(*parser).family).callback_bytes, callback_bytes, MAX_FAMILY_CALLBACK_BYTES) {
            (*parser).error = 43;
            return;
        }
    }
    let split_default = matches!(&kind,
        EventKind::StartDoctype { .. } | EventKind::EndDoctype
        | EventKind::EntityDeclaration { .. } | EventKind::AttlistDeclaration { .. }
        | EventKind::ElementDeclaration { .. } | EventKind::NotationDeclaration { .. }
    );
        ...
        match kind {
            EventKind::StartElement { name, attributes } => {
                ...
                if let Some(callback) = h.start_element {
                    ...
                    let strings: Vec<CString> = attributes
                        .into_iter()
                        .flat_map(|a| [cstring(a.name), cstring(a.value)])
                        .collect();
                    let mut attributes: Vec<*const c_char> =
            ...
            EventKind::TextDeclaration { version, encoding } => {
                if let Some(callback) = h.xml_decl {
                    callback(arg, cptr(&optional_cstring(version)), cstring(encoding).as_ptr(), -1);
                } else { handled = false; }
            }
            EventKind::ExternalEntityReference { context, system_id, public_id } => {
                if let Some(callback) = h.external {
                    let handler_arg = if (*parser).external_arg.is_null() { parser } else { (*parser).external_arg.cast() };
                    if callback(handler_arg, cptr(&optional_cstring(context)), cptr(&base), cstring(system_id).as_ptr(), cptr(&optional_cstring(public_id))) == 0 {
                        (*parser).error = 21;
                    }
                }
            }
            ...
            EventKind::EntityDeclaration {
                ...
                if let Some(callback) = h.entity_decl {
                    let len = value.as_ref().map_or(0, String::len) as c_int;
                    callback(
                        arg,
                        cstring(name).as_ptr(),
            ...
            EventKind::ElementDeclaration { name, model } => {
                if let Some(callback) = h.element_decl {
                    match content_model::allocate(&model) {
                        Ok(model) => callback(arg, cstring(name).as_ptr(), model),
    ...
    if !handled {
        ...
        unsafe {
            if !(*parser).pending_free && (*parser).handlers.default.is_some() {
                let raw = (*parser).core.current_raw().map(str::to_owned);
                if let Some(raw) = raw {
                    if split_default {
                        for fragment in DtdFragments(raw.as_str()) {
                            (*parser).default_pending.push_back(fragment.to_owned());
                        }
                    } else {
                        (*parser).default_pending.push_back(raw);
                    }
...
unsafe fn dispatch_default_fragment(parser: XML_Parser, raw: &str) -> bool {
    ...
    unsafe {
        if (*parser).pending_free || (*parser).state == 3 || (*parser).error != 0 { return false; }
        let Some(callback) = (*parser).handlers.default else { return false; };
        let arg = if (*parser).handler_arg_is_parser { parser.cast() } else { (*parser).user_data };
...
unsafe fn drain_default_fragments(parser: XML_Parser) {
    ...
    unsafe {
        while !(*parser).pending_free && (*parser).state != 3 && (*parser).error == 0 {
            if (*parser).handlers.default.is_none() { (*parser).default_pending.clear(); return; }
            let Some(fragment) = (*parser).default_pending.pop_front() else { return; };
            if !dispatch_default_fragment(parser, &fragment) { return; }
        }
    }
}
...
unsafe fn run_events(parser: XML_Parser) -> c_int {
    loop {
        ...
        unsafe { drain_default_fragments(parser); }
        ...
        let event = unsafe {
            ...
            match (*parser).core.next_event() {
                ...
                Err(error) => {
                    ...
                    if (*parser).pending_free { return ERROR; }
                    if (*parser).error != 0 { return ERROR; }
        ...
        unsafe {
            dispatch(parser, event.kind);
        }
    }
}

unsafe fn resolve_unknown_encoding(parser: XML_Parser) -> bool {
    // SAFETY: Called after the core returns its owned error, under the outer busy
    // guard. Only owned names and stack callback output survive the C call.
    unsafe {
        let Some(handler) = (*parser).handlers.unknown_encoding else { return false; };
        let Some(name) = (*parser).core.unknown_encoding().map(str::to_owned) else { return false; };
        let arg = (*parser).unknown_encoding_arg;
        let mut info = XML_Encoding { map: [-1; 256], data: ptr::null_mut(), convert: None, release: None };
        let accepted = handler(arg, cstring(name.clone()).as_ptr(), &mut info) != 0;
        if accepted && !(*parser).pending_free && (*parser).error == 0
            && (*parser).core.set_encoding_map(&name, info.map).is_ok()
        {
            release_encoding(parser);
        ...
        // Expat releases handler-provided data even when the callback rejects the
        // encoding or supplies an invalid map. No parser reference crosses release.
        if let Some(release) = info.release { release(info.data); }
        false
    }
}
...
pub unsafe extern "C" fn XML_Parse(
    ...
    unsafe {
        ...
        if !charge(&Arc::as_ref(&(*parser).family).input_bytes, len as usize, (*parser).config.limits.max_total_bytes) {
            (*parser).error = 43;
            return ERROR;
        }
        (*parser).busy = true;
...
pub unsafe extern "C" fn XML_GetBuffer(parser: XML_Parser, len: c_int) -> *mut c_void {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        unsafe {
            ...
            let remaining = (*parser).config.limits.max_total_bytes.saturating_sub(Arc::as_ref(&(*parser).family).input_bytes.load(Ordering::Relaxed));
            if len > remaining { (*parser).error = 43; return ptr::null_mut(); }
...
pub unsafe extern "C" fn XML_SetBase(parser: XML_Parser, base: *const c_char) -> c_int {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Copy input before replacing storage, supporting XML_GetBase input.
        unsafe {
            let base = if base.is_null() {
                None
            } else {
                Some(CStr::from_ptr(base).to_owned())
            };
            (*parser).base = base;
        }
        OK
...
pub unsafe extern "C" fn XML_GetIdAttributeIndex(parser: XML_Parser) -> c_int {
    ...
    unsafe { (*parser).core.id_attribute_index().and_then(|index| index.checked_mul(2)).and_then(|index| c_int::try_from(index).ok()).unwrap_or(-1) }
}
...
pub unsafe extern "C" fn XML_ExternalEntityParserCreate(
    ...
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: All parent state is copied before returning the child. The core
        // child owns its environment; only atomic resource budgets are shared.
        // No user callback is invoked while parent references exist.
        unsafe {
            if (*parser).pending_free || (*parser).child_depth >= MAX_EXTERNAL_DEPTH {
                (*parser).error = 43;
                return ptr::null_mut();
            }
            if !charge(&Arc::as_ref(&(*parser).family).children, 1, MAX_FAMILY_CHILDREN) {
                (*parser).error = 43;
                return ptr::null_mut();
            }
            let (Ok(context), Ok(encoding)) = (copy_string(context), copy_string(encoding)) else { return ptr::null_mut(); };
            let Ok(core) = (*parser).core.external_child(context.as_deref(), encoding.clone()) else { return ptr::null_mut(); };
            let position = core.position();
            let mut config = (*parser).config.clone();
            config.encoding = encoding;
            Box::into_raw(Box::new(XML_ParserStruct {
                user_data: (*parser).user_data,
                core, config, handlers: (*parser).handlers,
                handler_arg_is_parser: (*parser).handler_arg_is_parser,
                busy: false, pending_free: false, state: 0, error: 0,
                final_buffer: false, position, specified_attributes: 0,
                base: (*parser).base.clone(), buffer: Vec::new(), buffer_available: false,
                external_arg: (*parser).external_arg,
                unknown_encoding_arg: (*parser).unknown_encoding_arg,
                default_dispatch: false,
                default_pending: VecDeque::new(),
                family: Arc::clone(&(*parser).family),
...
pub unsafe extern "C" fn XML_SetReparseDeferralEnabled(parser: XML_Parser, enabled: u8) -> u8 {
    if parser.is_null() || enabled > 1 { return 0; }
    ...
    unsafe { (*parser).core.set_reparse_deferral_enabled(enabled != 0); }
    1
}
...
pub unsafe extern "C" fn XML_MemMalloc(_parser: XML_Parser, size: usize) -> *mut c_void {
    // SAFETY: malloc accepts every size and returns NULL on allocation failure.
    unsafe { malloc(size) }
}
...
pub unsafe extern "C" fn XML_MemRealloc(
    ...
    // SAFETY: Caller passes NULL or an allocation from this C memory API.
    unsafe { realloc(pointer, size) }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn XML_MemFree(_parser: XML_Parser, pointer: *mut c_void) {
    // SAFETY: Caller passes NULL or a live allocation from this C memory API.
    unsafe {
        free(pointer);
    }
}
...
pub unsafe extern "C" fn XML_FreeContentModel(_parser: XML_Parser, model: *mut XML_Content) {
    // SAFETY: Models are allocated in a single malloc block, including all children
    // and names. NULL is accepted; non-NULL must come from a content-model callback.
    unsafe {
        free(model.cast());
    }
}
