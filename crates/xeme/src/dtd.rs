...
use xeme_storage::{Allocator, String, TryClone, Vec, try_insert, try_push};
...
pub(crate) struct ConditionalState {
    ...
    ignored_bytes: usize,
    header: Option<xeme_storage::Box<composition::Header>>,
    declaration: Option<xeme_storage::Box<composition::Declaration>>,
    attlist: Option<xeme_storage::Box<attlist::State>>,
}
...
impl Parser {
    pub(crate) fn parse_doctype(
        ...
        self.emit(
            EventKind::StartDoctype(xeme_storage::try_box(
    ...
    fn notation_declaration(
        ...
        if emit {
            self.emit(
                EventKind::NotationDeclaration(xeme_storage::try_box(
    ...
    fn entity_value(
        ...
        let mut active = xeme_storage::HashSet::with_hasher_in(
            self.tables.parameter_entities.hasher().clone(),
            self.allocator,
        );
        let mut blockers = xeme_storage::HashSet::with_hasher_in(
            self.tables.parameter_entities.hasher().clone(),
            self.allocator,
        );
        ...
        loop {
            ...
            if !blockers_initialized {
                for name in declaration_parameters {
                    xeme_storage::try_set_insert(&mut blockers, name.as_str())?;
                }
            ...
            xeme_storage::try_set_insert(&mut active, name.as_str())?;
    ...
    fn attlist_attribute(
        ...
        self.emit(
            EventKind::AttlistDeclaration(xeme_storage::try_box(
