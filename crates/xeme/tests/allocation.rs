...
use xeme_storage::{Allocator, MemorySuite};
...
fn next_event(parser: &mut Parser) -> Result<Option<xeme::Event>, Error> {
...
fn workload(allocator: Allocator) -> Result<(), Error> {
    let mut parser = Parser::try_new_with_encoding_in(
        Config {
            ...
            name_rules: xeme::NameRules::FourthEdition,
            ..Config::default()
        },
        None,
        allocator,
    ...
    let mut retained: [Option<xeme::Event>; 5] = Default::default();
    while let Some(event) = next_event(&mut parser)? {
...
fn detached_start_frames_use_the_selected_suite_and_clear_on_every_failure() {
    ...
    check_allocations(|allocator| {
        frames(
            allocator,
            Some('|'),
            b"<r xmlns:p='urn:p'><n p:a='first' xml:lang='en'/><n p:a='next' a='plain'/></r>",
        )?;
