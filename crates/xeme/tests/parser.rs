...
fn text<T: From<xeme_storage::String>>(value: &str) -> T {
    xeme_storage::String::try_from_str_in(value, xeme_storage::Allocator::System)
        .unwrap()
        .into()
}
            ...
            if let EventKind::Text(text) = event.kind {
                ...
                if let Some(text) = pending.take() {
                    events.push(EventKind::Text(
                        xeme::Text::try_from_str_in(&text, parser.allocator())
                            .map_err(|_| ErrorKind::NoMemory)?,
                    ));
                }
...
fn external_entity_namespace_context_consumes_the_expansion_budget() {
    ...
    let context = loop {
        ...
        if let EventKind::ExternalEntityReference(declaration) = event.kind {
            let xeme::ExternalEntityReference { context, .. } =
...
fn external_entities_require_application_supplied_content() {
    ...
    .unwrap();
    assert!(events.iter().any(|event| matches!(event, EventKind::ExternalEntityReference(declaration) if matches!(declaration.as_ref(), xeme::ExternalEntityReference { system_id, .. } if system_id.as_deref() == Some("file:///etc/passwd")))));
    assert!(
        parse(
                ...
                let result = loop {
                    match parser.next_event() {
                        Ok(Some(event)) => {
                            if matches!(
                                event.kind,
                                EventKind::ExternalEntityReference(declaration) if matches!(declaration.as_ref(), xeme::ExternalEntityReference { context: None, .. }
                            )) {
                                let mut child =
...
fn foreign_dtd_requests_have_null_identifiers() {
    ...
    assert!(matches!(
        parser.next_event().unwrap().unwrap().kind,
        EventKind::ExternalEntityReference(declaration) if matches!(declaration.as_ref(), xeme::ExternalEntityReference {
            context: None,
            system_id: None,
            public_id: None,
            ..
...
fn external_subset_callback_occurs_before_a_root_is_available() {
    ...
    while let Some(event) = parser.next_event().unwrap() {
        ...
        {
            let xeme::ExternalEntityReference { system_id, .. } =
                xeme_storage::Box::into_inner(declaration);
...
fn a_failed_encoding_setter_preserves_autodetection() {
    use xeme_storage::{AllocationTracker, Allocator, with_tracking};
