...
Oriole is experimental. The supported trial is an opt-in Linux x86-64 CPython
...
- [C callback contracts](../crates/oriole_expat/README.md) define pointer lifetimes,
...
structure to the recipient and copies strings through its allocator. See the
...
```console
python3 tools/compatibility.py api --library /absolute/liboriole_expat.so \
  --reference /absolute/libexpat.so --source /absolute/expat-2.8.4 \
  --config /absolute/expat-build/expat_config.h --output /tmp/oriole-api
python3 tools/compatibility.py w3c --library /absolute/liboriole_expat.so \
  --reference /absolute/libexpat.so --source /absolute/xmlconf --output /tmp/oriole-w3c
python3 tools/compatibility.py differential --library /absolute/liboriole_expat.so \
  --reference /absolute/libexpat.so --output /tmp/oriole-differential
```
...
See [benchmarking](../benchmarks/HILLCLIMB.md) and [acceptance](../CONTRIBUTING.md#acceptance).
