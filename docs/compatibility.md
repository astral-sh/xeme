...
Confirmation takes 0.9776412× selected native real-file time and 0.9837734× selected CPython time, remaining 1.2659× and 1.0975× Expat. ElementTree is 1.1423× and pyexpat events 1.0544× Expat; nine of 24 Python conditions meet the roughly 1.10× goal. Native and ElementTree remain above it. Initial measurements remain separate. Four epochs retain 104 conditions, 2,496 workers, 265,080 samples and all four adverse conditions. Shared-host affinity and counters establish neither isolation nor statistical significance.
...
Six current-source Rust ASan/fuzz harnesses pass: 69,199 replay executions and 7,978,060 exploration executions, with no recorded failure artifacts. [Completed ASan evidence](validation/2026-09-12/shared-native-raw-text/qualification/asan/) records nine instrumented crate/harness invocations and 22 successful, reaped commands. Leak detection is disabled; bounded exploration is not exhaustive safety or installed-distribution validation. No current raw-view installed PBS result is claimed. Earlier-source sanitizer and installed-distribution evidence remains historical; ABI, acceptance, callback behavior, resource safety and installed-consumer validation are distinct gates.
