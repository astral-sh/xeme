...
The [outer-whitespace report](validation/2026-09-12/outer-whitespace-separated-dispatch/) binds runtime `6320d7b7` to the normal build and PR162 base `87acc5a9`. It adds a transient space/TAB proof outside the root, retains the original inner Text plan, and preserves uncertain-input fallback, callback boundaries, eager coordinates and accounting order. The ordinary suite passes 466 Rust tests across 35 groups, formatting and strict workspace/locked-benchmark Clippy.

The original API matrix now records **4,349 passes, 391 failures and no timeouts**. Only ordered rows 110 and 505 (`test_misc_input_2gb`, chunk 0, deferral 0/1) change from timeout 14 to pass 0 under the original three-second alarm; the other 4,738 outcomes are exact. Six C consumers pass. Strict shared/static CPython each retain 802 methods, 809 rendered outcomes and the same two grouping failures (raw exit 2); both separate semantic tests pass per linkage. The original assertions, selectors and resource ceilings remain unchanged.

Confirmation native time is roughly tied with PR162 (0.9995746× control, 1.2950× Expat); confirmation CPython time is 0.9926089× control and 1.1182× Expat. ElementTree is 1.1612× and pyexpat 1.0767× Expat; five of 24 Python conditions meet the roughly 1.10× goal. Both overall aggregates remain above it. Initial native/Python results remain separate, as do every generated and adverse condition. Four epochs retain 104 conditions, 2,496 workers and 265,080 samples. CPU0 affinity and shared-host counters establish neither isolation nor statistical significance.

C ASan/UBSan covers the six C consumers linked to uninstrumented Rust, with leak detection disabled. Strict CPython uses an explicit upstream cleanup backport; benchmark consumers use unmodified CPython sources. Current-source Rust ASan passes all six harnesses: 69,199 replay executions and 8,194,533 exploration executions. Nine required Rust crate/target invocations are ASan-instrumented; normal generic libraries are provenance controls only. Each harness uses a 600-second exploration bound, a ten-second input timeout, 1,536 MiB RSS limit and 65,536-byte maximum input, with leak detection disabled. All 22 recorded commands exited zero and their children were reaped. This bounded campaign does not establish exhaustive memory safety or installed-distribution compatibility. The first launch failed before fuzz compilation because its PATH omitted cargo/bin, so cargo-fuzz could not invoke bare rustc; all child processes were reaped and all six harnesses remained unstarted. The retry changes only the output path and fixes the launch PATH, retaining the runner, source, target directory, flags, six harnesses and bounds. An installed PBS trial of this source remains outstanding. ABI coverage, well-formedness, callback behavior, resource safety and installed-consumer validation remain distinct gates.

## Original failures and diagnostic limits

The [failure census](validation/2026-09-11/api-failure-census/) documents the original 391 assertion failures: 366 allocation retry/schedule expectations, 12 literal Expat version checks, 12 single-buffer resource-policy checks and one deferral allocation-growth assertion. The two original timeout rows are the only fixes in this change. An allocation assertion can stop execution before later semantic assertions; retaining it as a known failure does not prove those unreached assertions.
