# Skip payload construction for unobserved C comments

We previously copied, normalized, queued, and dispatched comment payloads even when neither a comment handler nor a default handler observed them. We forward handler interest into the parser and consume validated, unobserved C comments directly.

Complete comments without custom lexical conversion use the shortcut; observed comments, DTD processing, and the public Rust event API retain their existing paths. Syntax, source/entity work, positions, raw-context retention, reset, child inheritance, and handler replacement remain checked. Unobserved comments intentionally consume no callback-payload allowance because no payload is created.

Measured runtime [`5ce6d2b5067bf1be4548abf456ef15e344add46b`](https://github.com/astral-sh/xeme/commit/5ce6d2b5067bf1be4548abf456ef15e344add46b); base [`1957281e1422c9669ac9f7a68d350dab491eae85`](https://github.com/astral-sh/xeme/commit/1957281e1422c9669ac9f7a68d350dab491eae85). These identities describe the measured source even if later test, CI, or documentation commits advance the PR head; no byte-for-byte source-identity claim is made for those later heads. Ordinary O3/ThinLTO, one codegen unit, generic x86-64 builds use the same compiler; Ohm experimental defaults were disabled and each frozen build used fresh intermediates.

Shared library SHA-256: `89fb56f5ebf8a783cd6d204784ba75599ecc15c3352ba8c956b7bf2a61c7053d`. Static SHA-256: `5d635420c6c284d0ecc3e3327d4726d43b896b7265207930c5ff2f08c23da6cd`. Base shared SHA-256: `b364ffc5164f2057a1f339508e7be135ad6f6a74be9f1e347466b7586359cbf9`. Expat 2.8.4 control SHA-256: `7a333bc8ac92ff53b8fc1ecc68711802e7c5781f52d3d0592a6f0ee362d9f478`.
...
| --- | --- | --- | --- | --- | --- |
| native / tuning | 24 | 0.9934× | 1.1667× | 0.9939× | 1.1660× |
| native / observed-holdout | 20 | 1.0023× | 1.4215× | 1.0015× | 1.4176× |
| native / generated | 16 | 1.0118× | 1.7049× | 1.0104× | 1.7100× |
| python / tuning | 24 | 0.9912× | 1.1375× | 0.9941× | 1.1482× |
| python / observed-holdout | 20 | 0.9959× | 1.1755× | 1.0002× | 1.1813× |

Aggregate slowdowns occur in native/observed-holdout (+0.23% / +0.15% in epochs 1 / 2); native/generated (+1.18% / +1.04% in epochs 1 / 2); python/observed-holdout (-0.41% / +0.02% in epochs 1 / 2). Native observed-holdout time is 1.4215× / 1.4176× Expat's time in epochs 1 / 2. This change does not close the overall native gap on that corpus.
...
| --- | --- | --- | --- | --- |
| native/musescore/all | -3.37% | 1.5475× | -3.87% | 1.5449× |
| python/musescore/pyexpat-events | -2.34% | 1.1338× | -2.03% | 1.1447× |
| python/musescore/elementtree | -0.11% | 1.2655× | +1.47% | 1.2752× |

## Adverse observations

We retain all 208 condition rows, including 49 / 53 slower-than-base observations in epochs 1 / 2; 38 conditions are slower in both epochs. No adverse observations are dropped or treated as statistically significant.

| Consumer / corpus | Slower epoch 1 | Slower epoch 2 | Conditions / epoch |
| --- | --- | --- | --- |
| native / tuning | 14 | 11 | 24 |
| native / observed-holdout | 12 | 12 | 20 |
| native / generated | 12 | 10 | 16 |
| python / tuning | 5 | 8 | 24 |
| python / observed-holdout | 6 | 12 | 20 |
...
| --- | --- | --- |
| `native/vulkan/65536/namespaces-1` | +3.05% | +3.22% |
| `native/vulkan/65536/namespaces-0` | +2.71% | +2.54% |
| `native/qt/4096/namespaces-1` | +3.15% | +2.53% |
| `native/entities/4096/namespaces-0` | +2.53% | +3.34% |
| `native/qt/65536/namespaces-1` | +2.92% | +2.46% |

## Mechanism evidence

On MuseScore, namespaces off, 4,096-byte feeds, separately collected Callgrind Ir counts change from 354,319,026 to 341,403,558 (0.9635×, -3.65%). Four parses include warmup, parser lifecycle, and native callback hashing; callback signatures/counts match. These instruction counts are not a wall-time measurement.

## Compatibility and coverage

The workspace passes 543 default-feature tests across 38 suites; formatting and all-target Clippy pass. The retained workspace evidence records the default-feature run; no no-default-feature claim is made here. All ten frozen-library compatibility commands pass with source/library hashes unchanged. The original API suite retains 495 known Xeme failures among 4,740 rows, with 14 improvements against the reviewed baseline and 0 regressions; Expat retains 0 failures. Strict adapted allocation behavior passes 1,008 rows and 1,008 ownership reports per engine; Xeme has 0 live tracked blocks and 0 unexpected system failures. Its retry sweeps record 23,912 denied malloc calls and 1,637 denied realloc calls, not distinct allocation sites. W3C has 6,003 rows per engine, 960 mandatory catalog failures for Xeme and 960 for Expat, and 0 engine differences. All 2,730 differential semantic cases pass; exact observations retain 8 callback-stream and 84 final-position differences (exact_pass=false).
...
PR head [`9570d0d735dd90f6c48e5151471e7bcdbc1601b1`](https://github.com/astral-sh/xeme/commit/9570d0d735dd90f6c48e5151471e7bcdbc1601b1) has 18 successful checks and one intentional `Profile-guided library` skip: [passed CI run 1](https://github.com/astral-sh/xeme/actions/runs/34787909874). The exact check inventory and retained status/report hashes are in the [CI summary](https://github.com/astral-sh/xeme/blob/charlie/codex-perf-ignored-comments/docs/evidence/2026-09-13-ignored-comments-performance.json); the [CI report](https://github.com/astral-sh/xeme/blob/charlie/codex-perf-ignored-comments/docs/evidence/2026-09-13-ignored-comments-performance.md#ci) identifies the tested head. The frozen timing and local compatibility results remain bound to their measured runtime. Subsequent test and workflow fixes receive their own CI qualification, and later documentation commits are not implicitly covered by this earlier run. Later workflow changes update the exact Miri test inventory. A later comments-test fix refreshes registered callback user data after Rust assertions before invoking the child parser; production parser code is unchanged.
...
[Compact results](https://github.com/astral-sh/xeme/blob/charlie/codex-perf-ignored-comments/docs/evidence/2026-09-13-ignored-comments-performance.json), [all 208 conditions](https://github.com/astral-sh/xeme/blob/charlie/codex-perf-ignored-comments/docs/evidence/2026-09-13-ignored-comments-performance.csv), and [verified raw archive](https://github.com/astral-sh/xeme/releases/download/performance-followup-2026-09-13/performance-followup-2026-09-13.tar.zst) (SHA-256 `80222e8cadbf6d90a305ad23f4e06d0e99f662bb7ef7c019dab8c417fbf17ecf`). The archive contains complete raw reports, command logs, compatibility outcomes, source snapshots, and frozen shared/static libraries; its external sidecar records decompression and member-hash verification.
