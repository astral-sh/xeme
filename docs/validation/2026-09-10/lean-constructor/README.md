...
[report.json](report.json) records exact counts and scope. The original `summary.json` and following files are retained inside [evidence.tar.gz](evidence.tar.gz): `builds.json`, `build-source.json`, and `source.tar.gz` identify the source and local builds. No binaries are packaged. `independent-review.json` is the root agent's separate source review. The original upstream failures and all unsuccessful experiments remain included.

[files.json](files.json) verifies every archived member.
