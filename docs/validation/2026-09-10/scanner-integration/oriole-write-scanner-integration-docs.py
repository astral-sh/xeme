...
repo=Path('/home/dev-user/code/oss/oriole-scanner-integration');out=repo/'docs/validation/2026-09-10/scanner-integration'
text='''# Short positions and fallible string operations
...
readme=repo/'README.md';old=readme.read_text();s=old
start=s.index('| Vulkan registry |');end=s.index('\n\n',start);s=s[:start]+Path('/tmp/oriole-scanner-integration-table.md').read_text().rstrip()+s[end:]
s=s.replace('benchmarks/results/2026-09-10/coalesced-search/','benchmarks/results/2026-09-10/scanner-integration/')
pstart=s.index('The latest text-scanning change reduces');pend=s.index('\n\n',pstart)
s=s[:pstart]+"The latest position and string changes reduce elapsed time by 1.8% across the 24 native project conditions and 2.2% across the 24 actual CPython conditions versus `193e1278`. Oriole still takes 2.01× Expat's time natively and 1.42× through CPython. The two Wayland pyexpat conditions are faster than Expat. The [full report](docs/validation/2026-09-10/scanner-integration/) retains all conditions and samples, including the native and Python regressions."+s[pend:]
s=s.replace('docs/validation/2026-09-10/coalesced-search/) records 394 workspace tests','docs/validation/2026-09-10/scanner-integration/) records 395 workspace tests and one doc test').replace('these precede the latest text-scanning change','these precede the latest parser changes')
assert old[old.index('## Installation'):]==s[s.index('## Installation'):]
assert re.findall(r'^#+ .*$',old,re.M)==re.findall(r'^#+ .*$',s,re.M)
assert old[:old.index('| Project XML |')]==s[:s.index('| Project XML |')]
readme.write_text(s)
p=repo/'benchmarks/README.md';p.write_text(p.read_text().replace('results/2026-09-10/coalesced-search/','results/2026-09-10/scanner-integration/',1))
p=repo/'docs/compatibility.md';p.write_text(p.read_text().replace('validation/2026-09-10/coalesced-search/','validation/2026-09-10/scanner-integration/',1))
...
b=repo/'benchmarks/results/2026-09-10/scanner-integration';b.mkdir(parents=True,exist_ok=False)
for name in ['native-summary.json','python-summary.json']:shutil.copy2(out/name,b/name)
(b/'README.md').write_text('''# Position and string integration benchmarks
...
'''+ '| Project XML | Oriole | Expat | Oriole / Expat |\n| --- | ---: | ---: | ---: |\n'+Path('/tmp/oriole-scanner-integration-table.md').read_text()+'''
...
files=[{'path':str(p.relative_to(out)),'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(out.rglob('*')) if p.is_file() and p!=out/'files.json'];(out/'files.json').write_text(json.dumps({'files':files},indent=2)+'\n');print(len(files),'indexed files; README structure, warning and installation/license unchanged')
