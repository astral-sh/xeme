"""Portable records-only replay of completed Python campaigns; no target imports.

Use --root for a copy containing normal/ and pgo/ study directories. Original
absolute paths inside records identify artifacts; excluded binaries need not be
present. Their identities were checked by the separate preflight audit.
"""
...
cli.add_argument('--root',type=Path,default=Path('/tmp/oriole-arena-capacity-bound-python-study'))
cli.add_argument('--mode',choices=('normal','pgo'),required=True)
cli.add_argument('--output',type=Path,default=Path(__file__).parent)
args=cli.parse_args();D=args.root/args.mode;S=D/'screen'
original=Path('/tmp/oriole-arena-capacity-bound-python-study')/args.mode
pins={};aliases=[]
def read(p):
...
assert data['conditions']==pref['conditions']==load(D/'preparation.json')['conditions']
...
read(__file__)
out={'status':'passed_independent_raw_elapsed_reconstruction','mode':args.mode,'scope':'Portable saved records only; no binaries imported or executed, no elapsed rerun. Actual consumer/library identity hash readback is in separate independent preflight receipt 172385ca2d471dc21dce5f516e252206dac5d3f9548829f2781cd0afe26d3066.',
...
aliaspath=args.output/(args.mode+'-row-aliases.json');aliaspath.write_text(json.dumps(aliases,indent=2)+'\n');out['row_aliases_sha256']=hashlib.sha256(aliaspath.read_bytes()).hexdigest()
dest=args.output/(args.mode+'-review.json');dest.write_text(json.dumps(out,indent=2)+'\n')
