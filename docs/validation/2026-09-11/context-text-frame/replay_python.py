...
cli.add_argument('--root',type=Path,default=Path('/tmp/oriole-context-text-frame-fixed-python-study'))
...
original=Path('/tmp/oriole-context-text-frame-fixed-python-study')/args.mode
pins={};aliases=[]
def read(p):
...
read(__file__)
out={'status':'passed_independent_raw_elapsed_reconstruction','mode':args.mode,'scope':'Portable saved records only; no binaries imported or executed, no elapsed rerun. Actual consumer/library identity hash readback is in separate independent preflight receipt fe160729.',
