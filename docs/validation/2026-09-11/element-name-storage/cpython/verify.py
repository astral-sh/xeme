...
results={};csv_rows=[]
with tempfile.TemporaryDirectory(prefix='oriole-suffix-python-records-') as temp:
    ...
    delta={name:{'control':sources['selected'][name],'candidate':digest} for name,digest in sources['suffix'].items() if digest!=sources['selected'][name]}
    assert sorted(delta)==['crates/oriole/src/lib.rs','crates/oriole/tests/multibyte.rs']
    ...
    for mode in ('normal','pgo'):
        ...
        assert adaptation['candidate_source_manifest']['sha256']==sha((root/'builds/suffix/source.json').read_bytes())
        assert adaptation['candidate_pair_report']['sha256']==sha((root/'builds/suffix/pair-report.json').read_bytes())
        assert adaptation['selected_control_source_manifest']['sha256']==sha((root/'builds/selected/source.json').read_bytes())
        assert adaptation['selected_control_pair_freeze']['sha256']==sha((root/'builds/selected/pair-freeze.json').read_bytes())
        field,key=('normal_libraries','liboriole_expat.so') if mode=='normal' else ('pgo_libraries','use/liboriole_expat.so')
        for label,engine in [('suffix','candidate'),('selected','control')]:assert pairs[label][field][key]==adaptation['libraries'][engine]['sha256']
        ...
        for engine,consumer in build['consumers'].items():
            for module,command in zip(('pyexpat','_elementtree'),consumer['commands'],strict=True):
                ...
                expected_command=['cc','-shared','-fPIC','-O2',*['-I'+p for p in ('/home/dev-user/code/oss/oriole-suffix-element-names/include',inc,inc+'/internal',upstream+'/expat',upstream)],source_key,consumer['library'],'-Wl,-rpath,'+consumer['directory'],'-o',consumer['directory']+'/'+module+'.cpython-312-x86_64-linux-gnu.so']
    ...
    # Strict suite verification is appended below after inspecting its released review.
    strict_results={}
    for linkage in ('shared','static'):
        ...
        for role,identity in origin['origins'].items():
            assert identity['path']==f'/tmp/oriole-suffix-element-names-strict-cpython/{linkage}/'+role.split(':')[1]+'.cpython-312-x86_64-linux-gnu.so'
        ...
        assert summary['library_sha256']==pairs['suffix']['pgo_libraries'][library]
    ...
    for linkage in ('shared','static'):
        ...
        baseline=(root/'strict/historical-context'/linkage/'tests.log').read_text()
        ...
        strict_results[linkage].update({'reported_tests':802,'failures':failures,'skips':14,'rendered_verbose_lines':806,'rendered_outcomes_equal_historical_context':True})
out={'status':'passed_portable_records_replay','archive_files':len(expected),'archive_sha256':report['evidence_sha256'],'source_files':{'suffix':72,'selected':72},'modes':results,'strict':strict_results,'scope':'Saved source/build/consumer records and original raw worker arithmetic; no parser imports or original absolute-path reads. Actual compiler/library byte checks belong to the archived original reviews.'}
