"""Write claims from the complete retained Cell benchmark suite."""
...
report = '''# Serialized parser family budgets
...
(bench / 'README.md').write_text('''# Serialized parser family budgets
...
readme = readme.replace('These [native measurements](benchmarks/results/2026-09-11/c-library-thinlto/)', 'These [native measurements](benchmarks/results/2026-09-11/family-budget-cells/)')
start = readme.index('Building the C libraries with effective ThinLTO')
end = readme.index('\n\nOptional [profile-guided builds]', start)
readme = readme[:start] + "Using cells for the C interface's serialized family counters reduces elapsed time by 0.9% across the 24 native project conditions; actual CPython time is effectively unchanged. Both Oriole builds use effective ThinLTO. Oriole still takes 1.71× Expat's time natively and 1.31× through CPython. One Batik native condition and three Wayland CPython conditions are faster than Expat in this measurement; the native Batik and Wayland ElementTree margins are only 1.36% and 0.91% on a shared host. The [full report](docs/validation/2026-09-11/family-budget-cells/) retains all conditions, samples and regressions." + readme[end:]
readme = readme.replace('The [latest compatibility report](docs/validation/2026-09-10/start-frame-integration/) records 398 workspace tests', 'The [latest compatibility report](docs/validation/2026-09-11/family-budget-cells/) records 400 workspace tests')
(repo / 'README.md').write_text(readme)
(out / Path(__file__).name).write_bytes(Path(__file__).read_bytes())
print('Wrote Cell report, benchmark summary and README; warning, headings and license retained.')
