...
def literal(text):
 ...
 return ''.join(ast.literal_eval(q) for q in re.findall(r'"(?:\\.|[^"\\])*"',text))
a=literal(c.split('static const char long_default_root[]',1)[1].split(';',1)[0])
b=literal(u.split('START_TEST(test_nsalloc_long_default_in_ext)',1)[1].split('const char *text',1)[1].split(';',1)[0])
...
for engine in ['expat','shared','static']:
 ...
 run=(D/(engine+'-run.log')).read_text();assert run.strip()==('C ABI integration checks passed (expat_2.8.4)' if engine=='expat' else 'C ABI integration checks passed (oriole_compat_2.8.4)')
