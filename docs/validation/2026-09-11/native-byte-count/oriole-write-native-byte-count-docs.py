"""Describe the selected byte-count change from the frozen evidence package."""
...
assert summary['workspace']['all_target_tests'] == 407
table = (out / 'table.md').read_text().strip()
report = '''# Native source byte counts
...
[summary.json](summary.json), [build.json](build.json), [workspace.json](workspace.json) and [gates.json](gates.json) record the identities and checks. [evidence.tar.gz](evidence.tar.gz) preserves sources, commands, compiler logs, raw measurements, review receipts and earlier unsuccessful setup attempts. Its ARCHIVE_MEMBERS direct members and ARCHIVE_EXCLUDED binary exclusions are indexed in [archive-members.json](archive-members.json); SHA-256 is `ARCHIVE_SHA`. The packager read back every direct member and its original. Nested handoffs retain their own detailed readback receipts.
...
'''
report = report.replace('TABLE', table).replace('ARCHIVE_MEMBERS', str(summary['archive']['members'])).replace('ARCHIVE_EXCLUDED', str(summary['archive']['binary_exclusions'])).replace('ARCHIVE_SHA', summary['archive']['sha256'])
(out / 'README.md').write_text(report)
(out / Path(__file__).name).write_bytes(Path(__file__).read_bytes())
(bench / 'README.md').write_text('''# Native source byte counts
...
readme = readme.replace('These [native measurements](benchmarks/results/2026-09-11/detached-end-frames/)', 'These [native measurements](benchmarks/results/2026-09-11/native-byte-count/)')
start = readme.index('Detaching matched closing-tag names')
end = readme.index('\n\nOptional [profile-guided builds]', start)
readme = readme[:start] + "Inlining native source byte counts reduces elapsed time by 3.1% across the 24 native project conditions and 1.5% through CPython. Both Oriole builds use verified ThinLTO without PGO or alternate allocators. Oriole still takes 1.59× Expat's time natively and 1.27× through CPython. Generated controls improve by 4.0%; one CPython condition regresses by 0.59%. The [full report](docs/validation/2026-09-11/native-byte-count/) retains all conditions and samples." + readme[end:]
readme = readme.replace('The [latest compatibility report](docs/validation/2026-09-11/detached-end-frames/)', 'The [latest compatibility report](docs/validation/2026-09-11/native-byte-count/)')
assert readme[readme.index('## License'):] == before[before.index('## License'):]
assert [line for line in readme.splitlines() if line.startswith(('#', '>'))] == [line for line in before.splitlines() if line.startswith(('#', '>'))]
(repo / 'README.md').write_text(readme)
index = (repo / 'benchmarks/README.md').read_text()
index = index.replace('See the [current native and consumer measurements](results/2026-09-10/start-frame-integration/)', 'See the [current native and consumer measurements](results/2026-09-11/native-byte-count/)')
(repo / 'benchmarks/README.md').write_text(index)
print('Wrote report and current benchmark links; README structure, warning and license unchanged.')
