"""Package the unselected CDATA prefilter and saved results; no targets."""
import csv
import gzip
import hashlib
import io
...
BUILD = Path('/tmp/oriole-cdata-prefilter-pgo-study')
NATIVE = 'cdata-prefilter-native-study'
AUDIT = 'cdata-prefilter-independent-review/native'
REVIEWS = Path('/tmp/oriole-cdata-prefilter-independent-review')
WORK = Path('/home/dev-user/code/oss/oriole-cdata-prefilter')
...
def main():
    ...
    assert digest((REVIEWS / 'native/review.json').read_bytes()) == 'a158aa14709a035cbabfa76fc0e5ac4e459e467403b183a7aa1a9f8ecf762edc'
    assert digest((REVIEWS / 'build/review.json').read_bytes()) == '92c456b019f1ba4331b7345695d661ec16ec79a500419948dbb2fa3d81785486'
    freeze = read(BUILD / 'pair-freeze.json')
    assert freeze['count'] == 69 and freeze['status'] == 'frozen'
    assert digest((BUILD / 'pair-freeze.json').read_bytes()) == 'ba448403c6756e674a8009947383bd504c03c561564cdec6a76c793e16fdb69a'
    sources = {}
    for name, identity in freeze['files'].items():
        path = BUILD / name
        data = path.read_bytes()
        assert (digest(data), len(data)) == (identity['sha256'], identity['bytes'])
        sources['cdata-prefilter-pgo-study/' + name] = path
    sources['cdata-prefilter-pgo-study/pair-freeze.json'] = BUILD / 'pair-freeze.json'
    for prefix, folder in [(NATIVE, Path('/tmp/oriole-' + NATIVE)),
                           ('cdata-prefilter-independent-review', REVIEWS)]:
        for path in sorted(folder.rglob('*')):
            if path.is_file():
                sources[prefix + '/' + path.relative_to(folder).as_posix()] = path
    for folder_name in ('oriole-cdata-prefilter-patch', 'oriole-cdata-prefilter-source-assessment'):
        folder = Path('/tmp') / folder_name
        for path in sorted(folder.rglob('*')):
            if path.is_file():
                sources[folder_name.removeprefix('oriole-') + '/' + path.relative_to(folder).as_posix()] = path
    ...
    index = {'format': 'sha256-object-archive-v1', 'files': files, 'excluded_binaries_and_caches': excluded,
             ...
             'cache_scope': 'Candidate reusable pgo/targets cache is not traversed; the frozen69-file build inventory supplies artifact identities. Older study archives/histories are not copied.'}
    ...
    report = {'status': 'assembled_negative_experiment', 'decision': 'Unselected: PGO real-project results are effectively flat, with normal and generated regressions.',
              'source_manifest_sha256': digest((BUILD / 'source.json').read_bytes()), 'source_files': 72,
              'runtime_delta': ['crates/oriole/src/lib.rs'], 'test_delta': ['crates/oriole/src/lib.rs'],
              ...
              'local_tests': 439, 'actual_compiler_vectors': 9, 'generated_training_records': 864,
