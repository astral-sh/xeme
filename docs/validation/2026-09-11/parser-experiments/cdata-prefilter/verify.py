"""Verify a relocated CDATA-prefilter evidence package and replay saved native data.
...
def main():
    ...
    source = json.loads(get('cdata-prefilter-pgo-study/source.json'))['source_sha256']
    assert len(source) == 72
    source_seen = set()
    with tarfile.open(fileobj=io.BytesIO(get('cdata-prefilter-pgo-study/source.tar.gz')), mode='r:gz') as archive:
        for member in archive:
    ...
    assert report['source_manifest_sha256'] == sha(get('cdata-prefilter-pgo-study/source.json'))
