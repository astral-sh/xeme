"""Verify a relocated namespace-guard evidence package and replay saved native data.
...
def main():
    ...
    source = json.loads(get('default-namespace-guard-pgo-study/source.json'))['source_sha256']
    assert len(source) == 72
    source_seen = set()
    with tarfile.open(fileobj=io.BytesIO(get('default-namespace-guard-pgo-study/source.tar.gz')), mode='r:gz') as archive:
        for member in archive:
    ...
    assert report['source_manifest_sha256'] == sha(get('default-namespace-guard-pgo-study/source.json'))
