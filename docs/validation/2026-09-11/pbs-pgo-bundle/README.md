...
The [stable-toolchain PGO PBS distribution trial](../pgo-trial-results/) has completed: its shipped archive matches the fresh profile-use build, the distribution validator and installed identity pass, and glibc 2.17 completes 1,024 threaded parses. Both strict XML gates retain the same two callback assertions as the normal baseline. The earlier local build below used Ohm with experimental defaults disabled; neither trial adds a new elapsed benchmark.

| Evidence | Result |
| --- | --- |
| [Stable PGO PBS distribution](../pgo-trial-results/) | Exact profile-use archive shipped; validator, identity and 1,024 glibc 2.17 parses pass; both strict XML gates retain two assertions. |
...
The [report](report.json), [workflow check record](workflow-check.json) and [executed check source](workflow-check.py) describe the routing probes and source boundaries. The [workflow review](workflow-independent-review.json) checks the opt-in routing, and the [archive review](archive-root-review.json) independently checks both evidence archives. [File hashes](files.json) identify the report's publication inputs. The [stable PGO report](../pgo-trial-results/report.json) supplies the installed archive, strict XML and glibc results. The two XML assertions remain unresolved, and application measurements of that exact produced distribution remain separate evidence before assigning this configuration production readiness.
