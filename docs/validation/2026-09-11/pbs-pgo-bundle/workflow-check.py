...
condition = "${{ (github.event_name == 'workflow_dispatch' && inputs.pgo) || (github.event_name == 'pull_request' && startsWith(github.head_ref, 'charlie/codex-oriole-pbs-pgo-')) }}"
assert job['env']['ORIOLE_PGO'] == condition
assert steps['Prepare optional PGO tools']['if'] == "${{ env.ORIOLE_PGO == 'true' }}"
assert 'env' not in steps['Build the frozen Oriole bundle']
...
with tempfile.TemporaryDirectory(prefix='oriole-workflow-') as directory:
    ...
    for selected in (False, True):
        ...
        environment = {'PATH': str(binary) + ':/usr/bin:/bin', 'CALLS': str(calls_path), 'FAKE_SYSROOT': str(sysroot), 'GITHUB_ENV': str(github_env), 'RUNNER_TEMP': str(temporary), 'GITHUB_WORKSPACE': str(temporary / 'workspace with spaces'), 'ORIOLE_PGO': str(selected).lower()}
