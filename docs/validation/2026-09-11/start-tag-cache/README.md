...
The archived native auditor and its original review retain descriptive text about the prior serialized-accounting experiment, nominal core types, nine runtime files and unchanged C fixtures. That text is stale: the executable source checks bind this cache candidate to five changed files in the 72-file set and four in the 69-file pipeline set, including inherited integration coverage only in the former. Original receipts are preserved; the source bindings, worker arithmetic and this report state the current scope.
