...
ROOT = Path('/tmp/oriole-matched-native-end-raw-study')
NATIVE = ROOT / 'native'
...
assert libraries['candidate']['sha256'] == '5ca9644b5d782a6beaae7a8b1ddd16045480d4566b7eb77e317c7ac25a2258fe'
...
assert source['base'] == '008d818237fe0d6f8c04a000a98ad69ab5780ceb' and len(source['sha256']) == 74
assert binding['source_count'] == 74 and set(binding['source_delta']) == {'crates/oriole/src/lib.rs', 'crates/oriole/tests/allocation.rs', 'crates/oriole_expat/src/tests.rs'}
assert binding['tests'] == {'passed':476,'groups':35,'failures':0,'ignored':0}
fixture = 'crates/oriole_expat/src/context_text_tests.rs'
assert set(binding['compiled_control_source_delta']) == set(binding['source_delta']) | {fixture}
assert binding['control_fixture_correction'] == adaptation['control_fixture_correction']
assert source['sha256'][fixture] == binding['control_fixture_correction']['corrected_sha256']
assert binding['source_manifest_sha256'] == sha(ROOT / 'source.json')
assert source['sha256']['crates/oriole/src/lib.rs'] == '4c50a28a0ba7fa8ac3cf725fee2626cf382a22022b226901084f68e4cfda721e'
...
assert (old / 'run.py').read_bytes() == (NATIVE / 'run.py').read_bytes()
...
result = {'status': 'passed_independent_saved_normal_native_reconstruction',
          ...
          'reader_adaptation': 'Reuse the completed raw-view normal reader with source, library and output bindings for matched-native-End raw range. Full raw reconstruction unchanged. Protocol preparer adapted this reader; root owns execution and independent review. Original raw reconstruction, worker counts, all ratios and adverse conditions unchanged.',
          ...
          'limitations': ['All28 conditions and every adverse result retained. Ratios use medians of seven paired process-median ratios, then equally weighted geometric means.', 'One normal-release campaign on this host, without statistical-significance or whole-application claims.', 'The original native driver explicitly dlopens the recorded library path; hashes and actual argv are checked, with no new dladdr origin probe.', 'Callback hashes merge text fragments; equality is not exact callback fragmentation compatibility.', 'The earlier preparation receipt remains historically labeled unexecuted; completed controller and rawworker files are execution evidence.', 'Only this matched-native-End-raw normal native campaign is reconstructed here. Python preparation and timing are conditional on a native gain and are outside this evidence.']}
