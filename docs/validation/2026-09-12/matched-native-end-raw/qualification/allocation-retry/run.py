"""Held root-only 300-row allocation-retry diagnostic; no existing output is reused."""
...
assert H(P/'retry-ceilings.patch')==prepared['retry_patch_sha256']
...
try:
    ...
    report['all_diagnostic_contexts_passed']=report['selection_complete'] and report['library_origin_verified'] and not report['commands'][-1]['outer_timed_out'] and report['commands'][-1]['exit']==0 and all(r['outcome']=='pass' and r['code']==0 for r in rows)
