...
# Prove the exact two-assertion observation change, including the whole-file inverse.
assert H(P/'diagnostic.patch')==p['diagnostic_patch_sha256']
assert H(P/'observation-transform.json')==p['observation_transform_sha256']
t=J(P/'observation-transform.json')
before=(base/'adapted/nsalloc_tests.c').read_text();after=(D/'adapted/nsalloc_tests.c').read_text()
assert H(D/'adapted/nsalloc_tests.c')==t['source_sha256']
assert after.count(t['new_block'])==1 and after.count(t['include_new'])==1
assert after.replace(t['new_block'],t['old_block']).replace(t['include_new'],t['include_old'])==before
...
for block in re.finditer(r'^ORIOLE_BEGIN\t([^\t\n]+)\t([^\t\n]+)\n(.*?)^ORIOLE_RESULT\t\1\t\2\t([^\n]+)\n',log,re.M|re.S):
    for status,error,count in re.findall(r'^ORIOLE_EMPTY_BUFFER_OBSERVATION\tstatus=(-?\d+)\terror=(-?\d+)\tallocation_count=(-?\d+)$',block[3],re.M):
        observations.append(dict(context=block[1],test=block[2],status=int(status),error=int(error),allocation_count=int(count)))
observation_complete=[(x['context'],x['test']) for x in observations]==expected
assert observations==r['empty_buffer_observations'] and observation_complete==r['observation_selection_complete']
all_pass=complete and observation_complete and not r['commands'][1]['outer_timed_out'] and r['commands'][1]['exit']==0 and all(x['outcome']=='pass' and x['code']==0 for x in rows)
assert all_pass==r['all_diagnostic_contexts_passed']
result=dict(status='passed_saved_raw_readback',targets_executed=False,runtime_commit=p['runtime_commit'],base_commit=p['base_commit'],candidate_patch_sha256=p['candidate_patch_sha256'],candidate_library_sha256=p['candidate_library_sha256'],original_api_unchanged=True,original_tail_assertions_unchanged=True,allocation_result_assertions_observed=2,empty_buffer_observations=observations,observation_selection_complete=observation_complete,expected_configurations=12,observed_results=len(rows),complete=complete,all_diagnostic_contexts_passed=all_pass,outcomes=r['outcomes'],nonpasses=[x for x in rows if x['outcome']!='pass'],controller_report_sha256=H(R/'report.json'),preparation_sha256=H(P/'prepared.json'),reader_sha256=H(__file__),limits=p['limits'],limitations=p['limitations'])
