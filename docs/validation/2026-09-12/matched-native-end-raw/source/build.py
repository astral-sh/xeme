"""Ordinary checks/build for the matched-native-end-raw candidate; no benchmarks."""
import hashlib
import json
import os
...
TARGET = Path('/home/dev-user/.cache/oriole/matched-native-end-raw-target')
...
assert base == prepared['base'] == '008d818237fe0d6f8c04a000a98ad69ab5780ceb'
...
assert sha(PREP / 'inputs.json') == inputs['inputs_sha256']
assert sha(PREP / 'bind_build.py') == inputs['binder_sha256']
assert prepared['baseline_sha256'] == parent['source74']
control = Path('/tmp/oriole-core-owned-text-raw-study')
old_binding = json.loads((control / 'build-binding.json').read_text())
assert sha(control / 'source.json') == parent['control_source_sha256']
assert sha(control / 'build.json') == parent['control_build_sha256']
assert sha(control / 'build-binding.json') == parent['control_binding_sha256']
assert old_binding['candidate_libraries'] == parent['control_libraries']
