...
adaptation = json.loads(Path('/tmp/oriole-native-start-end-namespace-declaration-study/python/adaptation.json').read_text())
assert adaptation['status'] == 'prepared_only'
...
phase = sys.argv[1]
common = ['--kind', 'python', '--build', '/tmp/oriole-native-start-end-namespace-declaration-study/python/consumers/build.json', '--input', '/home/dev-user/code/oss/oriole/benchmarks/projects/corpus-manifest.json', '--output', str(out / 'screen')]
if phase == 'prepare':
    jobs = [
...
report = {'status': 'incomplete', 'jobs': [], 'controller_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
          'scope': 'Six previously built O2 modules reused in original directories; zero compiles. Fresh canonical preflights on CPU3; separate confirmation elapsed study with CPU0 affinity on a shared host. Worker and measurement boundaries unchanged. Per-worker300s and aggregate timing1200s retained. Process group killed on outer timeout/interruption.'}
def save():
