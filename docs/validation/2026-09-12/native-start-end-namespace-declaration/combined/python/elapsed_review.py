...
D=Path('/tmp/oriole-native-start-end-namespace-declaration-study/python');S=D/'screen';original=D
...
binding=load(D/'preflight-review.json')
...
report={'status':'passed','mode':'normal','scope':'Saved-only final reconstruction using the selected independent raw-worker arithmetic. Four reused plus two fresh compiler vector/source bindings, all module and dladdr parser origins, canonical outputs, seeded worker order, sample arithmetic and summaries verified. No target execution or timing rerun.',
...
csvpath=D/'normal-conditions.csv'
