...
try:
    ...
    assert set(delta) == {'crates/oriole/src/arena.rs', 'crates/oriole/src/dtd.rs', 'crates/oriole/src/dtd/attlist.rs', 'crates/oriole/src/encoding.rs', 'crates/oriole/src/lib.rs', 'crates/oriole/tests/allocation.rs', 'crates/oriole_expat/src/context_text_tests.rs', 'crates/oriole_expat/src/lib.rs', 'crates/oriole_expat/src/tests.rs'}
    ...
    assert set(subprocess.check_output(['git','diff',base,'--name-only'],cwd=root,text=True).splitlines()) == set(delta)
    ...
    target = Path('/home/dev-user/.cache/oriole/core-owned-text-raw-target/x86_64-unknown-linux-gnu/release')
    ...
    assert old['libraries']['liboriole_expat.so'] == 'ce0a648ccd0c661e88e9c29643bb1d3893abfac3976ebc603b28f40eb0623229'
    ...
    report.update(status='passed', base=base, source_count=74, source_delta=delta,
                  ...
                  complete_patch_sha256=sha(out / 'candidate.patch'), check_scope='Full workspace tests/fmt/all-target strict Clippy; also locked inprocess benchmark fmt/Clippy. Same seven ordinary commands as selected whitespace control.', comparison_scope='Core-owned NativeText raw ranges composed with the unselected single-input-owner dependency versus selected whitespace normal ce0a64. Exactly nine source/test files differ in the main74 map; manifests/dependencies/auxiliary locks are unchanged. No frame-reservation, EmptyEnd, cold-span or scalar-name change. No parser/compiler/benchmark target executed by this saved binder.', build_sha256=sha(out / 'build.json'),
