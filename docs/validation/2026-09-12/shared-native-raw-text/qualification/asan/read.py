...
P = Path('/tmp/oriole-core-owned-text-raw-asan-preparation')
S = Path('/tmp/oriole-core-owned-text-raw-asan-study')
W = Path('/home/dev-user/code/oss/oriole-core-owned-text-raw')
OUT = Path('/tmp/oriole-core-owned-text-raw-asan-independent-review.json')
TARGETS = ['parse', 'streaming', 'ffi', 'ffi_family', 'multibyte', 'value_family']
RUNTIME = None
BASE = '2c4d21647efdefe0b7da977ca206b8552c32f0f3'
RELEASED_MANIFEST = None
HELD_MANIFEST = '2cb5526ff1f1e7f4890ca90a92d2e9df0c23b2b3ab903c337bf8c886f9a95b45'
RUNNER = 'ff714fb14b9c70210723a73b60d807cdcbe44d0eb0eaa0484d1da53104582a4e'
SOURCE = '05550226c63ecefdb8607f04a23efedaadcf77e449207f18e7be7c31b2426b38'
BINDING = 'b2027cd81bd1dc5830d338d757e8d2e7d77ee3758029e7443967f3da0ff23975'
pins = {}
...
def audit():
    ...
    restored = json.loads(json.dumps(m)); restored['release']['status'] = 'held'
    ...
    assert m['target_dir'] == '/home/dev-user/.cache/oriole/core-owned-text-raw-asan-target'
    ...
    assert binding['status'] == 'passed' and binding['tests'] == dict(passed=475, groups=35, failures=0, ignored=0)
    assert binding['candidate_libraries'] == m['release']['normal_libraries']
    assert binding['candidate_libraries']['liboriole_expat.so'] == 'e59d89d6e21b92042b8358bd8ceef45b0a60404c61c9aa06ccf2e85a00f1c7f6'
    ...
    assert m['release']['source_commit'] is None and m['release']['base_commit'] == BASE
    assert sorted(binding['source_delta']) == m['release']['source_delta'] and len(binding['source_delta']) == 9
    ...
    assert producer['source_identity'] == {'source_commit': None, 'base_commit': BASE,
    ...
    assert totals['initial_inputs']==69196
...
result={'status':'incomplete','runtime_commit':RUNTIME,'base_commit':BASE,'released_manifest_sha256':RELEASED_MANIFEST,
    ...
    'limitations':['Rust ASan for the three Oriole crates and six harnesses, using the external Clang runtime. detect_leaks=0; system libraries and Rust standard library are not claimed instrumented.',
        'Six retained seed corpora plus six source-derived FFI inputs are replay/exploration inputs, not exhaustive safety or branch-coverage proof. No speed claim.',
