...
def main():
    ...
    release = m['release']
    assert release['source_commit'] is None
    ...
    assert sorted(binding['source_delta']) == release['source_delta'] and len(release['source_delta']) == 9
    head = git('rev-parse', 'HEAD').decode().strip()
    assert head == release['base_commit']
    patch = git('diff', '--no-ext-diff', '--binary', release['base_commit'], '--', *sorted({**source['sha256'], **binding['auxiliary_files']}))
    ...
    source_identity = {'source_commit': None, 'base_commit': head, 'candidate_patch_sha256': patch_hash,
