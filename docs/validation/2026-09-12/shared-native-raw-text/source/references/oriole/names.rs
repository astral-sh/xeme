//! XML 1.0 character and name productions.
...
/// XML edition used to validate names, including names in DTDs and references.
///
...
impl NameRules {
    ...
    pub(crate) fn is_name(self, name: &str) -> bool {
        ...
        chars.next().is_some_and(|c| self.is_name_start(c)) && chars.all(|c| self.is_name_char(c))
    }
...
fn in_ranges(c: char, ranges: &[(u16, u16)]) -> bool {
    let Ok(value) = u16::try_from(u32::from(c)) else {
        return false;
    };
