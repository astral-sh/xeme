...
root=pathlib.Path('/tmp/oriole-namespace-name-proof-benchmark');commands=json.loads((root/'commands.json').read_text());phase=sys.argv[1]
...
try:
 if phase=='native':
  build=pathlib.Path('/tmp/oriole-namespace-name-proof-study/build.json');data=json.loads(build.read_text());assert data['status']=='passed'
  lib='/tmp/oriole-namespace-name-proof-study/normal/liboriole_expat.so'
