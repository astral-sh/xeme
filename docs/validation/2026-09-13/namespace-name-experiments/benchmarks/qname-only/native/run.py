...
assert json.loads((P.parent/'binding.json').read_text())['status']=='passed'
report={'status':'incomplete','commands':[],'controller_sha256':sha(P/'native_screen.py'),'wrapper_sha256':sha(__file__),'scope':'Sequential preflight CPU5 then native elapsed with CPU0 affinity; one attempt each, original90-second workers and1200-second aggregate timing bound. Controller process-group cleanup retains failure logs; no tuning/rerun.'}
