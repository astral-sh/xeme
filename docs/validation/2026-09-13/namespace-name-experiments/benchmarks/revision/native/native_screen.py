"""Fixed three-engine planned namespaced-start native screen."""
...
root = Path('/tmp/oriole-namespace-binding-revision-benchmark/native')
...
libraries = {name: Path(entry['path']) for name, entry in binding['libraries'].items()}
for name, path in libraries.items():
    assert hashlib.sha256(path.read_bytes()).hexdigest() == binding['libraries'][name]['sha256']
