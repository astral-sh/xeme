...
root=pathlib.Path('/tmp/oriole-namespace-binding-revision-benchmark');commands=json.loads((root/'commands.json').read_text());phase=sys.argv[1]
...
try:
 if phase=='native':
  build=pathlib.Path('/tmp/oriole-namespace-binding-revision-study/build.json');data=json.loads(build.read_text());assert data['status']=='passed'
  lib='/tmp/oriole-namespace-binding-revision-study/normal/liboriole_expat.so'
