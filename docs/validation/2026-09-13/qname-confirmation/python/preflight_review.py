...
D=Path('/tmp/oriole-namespace-qname-only-confirmation/python');S=D/'screen';pins={}
def read(p):
...
assert c['status']=='passed' and c['controller_sha256']==sha(D/'run.py') and len(c['jobs'])==1
assert [job['label'] for job in c['jobs']]==['preflight']
for job in c['jobs']:
...
assert b['reuse']=={'path':'/tmp/oriole-namespace-qname-only-benchmark/python/reuse.json','sha256':sha(D/'reuse.json')}
...
result={'status':'passed','mode':'normal','targets_executed':False,'scope':'Six original compile vectors reused without recompilation, and fresh worker-reported module paths plus in-process dladdr parser DSO origins; no module loading or new ELF execution here. These 24 benchmark canonical comparisons coalesce adjacent character callbacks and do not replace the strict CPython suite.',
