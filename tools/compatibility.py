...
def check_w3c(directory: Path, baseline: dict) -> dict:
    ...
    for engine in ("reference", "oriole"):
        ...
        rows[engine] = current
    differences = []
    for expected, actual in zip(rows["reference"], rows["oriole"], strict=True):
        ...
        if semantic(expected) != semantic(actual):
            differences.append({"reference": expected, "oriole": actual})
    return {
...
def main() -> int:
    ...
    libraries = {
        ...
        for label, path in (("oriole", args.library), ("reference", args.reference))
    }
    hashes = {
