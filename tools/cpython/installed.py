"""Validate the actual installed Oriole CPython with the same narrow exception.
...
def main() -> int:
    ...
    if not pyexpat.EXPAT_VERSION.startswith("oriole_"):
        parser.error("installed interpreter did not load Oriole")
    gate_path = Path(__file__).with_name("run.py")
    gate = runpy.run_path(str(gate_path), run_name="oriole_cpython_gate")
