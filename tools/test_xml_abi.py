...
class LibraryIsolationTests(unittest.TestCase):
    def test_internal_calls_use_the_selected_library(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for name, source in {
                "global": "int oriole_probe_owner(void) { return 111; }",
                "selected": """
                    int oriole_probe_owner(void) { return 222; }
                    int oriole_probe_result(void) { return oriole_probe_owner(); }
                """,
            }.items():
                path = root / f"{name}.c"
...
selected.oriole_probe_result.argtypes = []
selected.oriole_probe_result.restype = ctypes.c_int
print(selected.oriole_probe_result())
"""
